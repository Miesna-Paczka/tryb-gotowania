...
/* `zajawka` DOPISANA 2026-08-25, decyzja operatora. Do tej daty lead nie miał
   w pliku źródłowym żadnego miejsca: szesnaście przepisów miało go WYŁĄCZNIE
   w CMS-ie, z importu 19.08, więc repozytorium nie posiadało jedynej powierzchni
   prozy, jaką ma strona przepisu (`tov-przepisy.md:13`).

   KOSZT TEJ NIEOBECNOŚCI, ZMIERZONY: partia 20 nowych przepisów wyszła bez leadu
   i **żadna bramka nie mogła tego zgłosić** — pole spoza schematu jest niewidzialne
   dla kontroli sterowanej schematem. Wszystkie zielone światła były prawdziwe
   i wszystkie milczały o właściwej rzeczy (`learnings.md` L-009).

   Sekcja jest WYMAGANA, jak każda inna: brak `[zajawka]` to błąd źródła, nie uwaga.
   To jest cała poprawka — nie da się już wyprodukować przepisu bez leadu i nie
   zauważyć tego. Stoi na pierwszym miejscu, bo kolejność SEKCJI jest kolejnością
   czytania przez człowieka, a lead czyta się pierwszy. */
