 ...
 * dokumentacja zamknięta §3.5 i raz kosztowała zepsute środowisko testowe.
 *
 ...
 * produkcyjną i zawsze decyzją.
 *
 * NIE ZAPISUJE TEŻ `name` ANI `slug` — i to nie jest przeoczenie, tylko granica
 * kierunku prawdy. `PATCH` w sekcji 5 wysyła wyłącznie `{ fieldData }` złożone
 * z pól `*-html`, wartości pochodnych i `parser-url`. Pola natywnie webflowe
 * (tytuł, adres) prowadzi CMS; repo jest tam LUSTREM i przyjmuje je narzędziem
 * `przyjmij-slugi-z-cms.mjs`. Dwa skutki, oba trzeba znać:
 *   · zmiana adresu w Webflow NIE zostanie tu nadpisana (żaden push nie przeniesie
 *     żywej strony) — ale repo trzeba o niej powiadomić, inaczej wskazuje 404;
 *   · redakcja TYTUŁU w repo NIE DOJEDZIE na stronę tym łańcuchem. Wymaga zapisu
 * `fieldData.name` osobno i publikacji — czyli decyzji.
 *
 ...
 * z sekretem (decyzja). Sesja nie widzi jednak Pages:
 ...
 *
 * `--wykonaj` (maszyna lokalna, jest sieć) — POBIERA każdy ładunek z Pages
...
import { POLE as POLE_JSONLD, dokument, blok, kontrole as kontroleJsonLd } from './generuj-jsonld.mjs';
...
/* ------------------------------------------------- pole `json-ld-rich`
   Blok Recipe składa ten sam model parsera, który buduje `*-html`, więc nie
   ma jak się rozjechać z treścią strony. Skalary spoza parsera (tytuł, adres,
   opis meta, zdjęcie, kategoria, kuchnia, data) prowadzi CMS i przyjmuje je
   lustro `pola-z-cms.json` — bez świeżego lustra ten skrypt STOI, zamiast
   wypchnąć blok z nieaktualnym adresem kanonicznym.

   Bramki `kontrole()` biegną tu jeszcze raz, mimo że przeszły w generatorze:
   tam sprawdzały plik, tu sprawdzają dokładnie te bajty, które lecą do CMS-u. */
const LUSTRO_CMS = path.join(KORZEN, 'lancuch-html', 'pola-z-cms.json');
if (!fs.existsSync(LUSTRO_CMS)) {
  console.error(`brak ${LUSTRO_CMS} — lustro pól CMS jest wejściem bloku JSON-LD, nie opcją`);
  process.exit(1);
}
const lustroCms = JSON.parse(fs.readFileSync(LUSTRO_CMS, 'utf8'));

/* ------------------------------------------------- pola źródłowe wprost
   Pięć pól, które do 2026-08-24 wypełniała RĘKA, choć repo ma je w całości.
   Trzy skalary z `[meta]` i dwa łańcuchy mikroskładni odżywczej: szablon czyta
   je z CMS (`data-porcje-bazowe`, `data-czas`, `data-mp-odz-zrodlo-*`), a łańcuch
   ich nie zapisywał — więc każda redakcja pliku wymagała drugiego, ręcznego ruchu
   w Webflow. Przy 16 przepisach to była niewygoda; przy setkach to jest klasa awarii.

   To NIE jest zmiana kierunku prawdy, tylko domknięcie tego, który już obowiązuje:
   wszystkie pięć siedzi w `ladunek()` od migracji 2026-08-19, czyli repo je posiada.
   Komentarz nad `MAKRO` w `generuj-html.mjs` mówił o nich „sześć pól, które w CMS
   wypełnia się dziś ręcznie, a są w całości wyliczalne" — cztery z tamtej szóstki
   łańcuch pisał od początku, te pięć zostało pominiętych.

   SKUTEK, KTÓRY TRZEBA ZNAĆ: od teraz ręczna zmiana któregokolwiek z nich w Webflow
   zostanie nadpisana przy najbliższym przebiegu. Tak samo jak przy polach `*-html`.
   Chcesz inny czas albo inną wagę porcji — popraw `przepisy/<slug>.txt`.

   `liczba-porcji` NIE JEST TUTAJ i nie będzie: pola o tej nazwie nie ma już
   w kolekcji (zmierzone 2026-08-24), a widełki „2–3 porcje" niosą informację
   redakcyjną, której `porcje-bazowe` nie wyraża — patrz komentarz w `zrodlo.mjs`. */
const POLA_ZRODLOWE = {
  /* `zajawka` dopisana 2026-08-25 razem z sekcją w `zrodlo.mjs`. Idzie tędy,
     a nie przez `POLA_HTML`, bo jest zwykłym tekstem, nie polem RichText —
     tą samą drogą co `wartosci-odzywcze`. */
  zajawka: (z) => z.pola.zajawka,
...
function fieldDataDla(w) {
  const d = dokument({ zrodlo: w.zrodlo, item: w.itemId }, lustroCms);
  const tekst = blok(d);
  const zle = kontroleJsonLd(lustroCms[w.itemId].slug, d, tekst);
  if (zle.length) {
    console.error(`✗ ${w.zrodlo.meta.slug}: blok JSON-LD nie przechodzi bramek:\n  ` + zle.join('\n  '));
    process.exit(1);
  }
  const zrodlowe = Object.fromEntries(
    Object.entries(POLA_ZRODLOWE).map(([pole, we]) => [pole, we(w.zrodlo)]));
  return { ...w.pola, ...w.pochodne, ...zrodlowe, 'parser-url': w.ladunek.url, [POLE_JSONLD]: tekst };
}
...
if (przezMcp) {
  ...
  console.log('  5. NIE publikuj. Publikacja itemu jest decyzją ręczną.');
  console.log('\nUWAGA: pole „slug" w payloadzie jest ETYKIETĄ do logu, nie zapisem —');
  console.log('       `update_collection_items` dostaje wyłącznie `fieldData`. Adres i tytuł');
  console.log('       prowadzi CMS; rozjazd sprawdza `przyjmij-slugi-z-cms.mjs <zrzut>`.');
...
if (!wykonaj) {
  console.log(`\n[SUCHY BIEG] zapisałbym po ${Object.keys(doZapisu[0].fieldData).length} pól w każdym z ${doZapisu.length} itemów:`);
...
console.log('Itemy zostają WERSJAMI ROBOCZYMI. Publikacja jest decyzją ręczną.');
