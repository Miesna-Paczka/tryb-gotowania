...
import { POLE as POLE_JSONLD, dokument, blok } from './generuj-jsonld.mjs';
...
const LUSTRO_CMS = path.join(path.dirname(new URL(import.meta.url).pathname), 'pola-z-cms.json');
const lustroCms = fs.existsSync(LUSTRO_CMS) ? JSON.parse(fs.readFileSync(LUSTRO_CMS, 'utf8')) : null;
...
for (const { slug, item: id, zrodlo } of zrodla()) {
  ...
  // --- 4. JSON-LD -------------------------------------------------------
  /* Pole `json-ld-rich` jest jedynym miejscem w CMS, gdzie treść przepisu leży
     jako DANE, a nie jako HTML — i jedynym, które czytają roboty nierenderujące
     JS. Rozjazd znaczy tu, że strukturalny opis mówi co innego niż strona.

     Porównanie idzie po całym bloku, znak w znak, a nie po kluczach dokumentu:
     blok, który się parsuje i ma komplet kluczy, nadal może mieć zjedzony ogon
     kroku — a to jest dokładnie ta klasa uszkodzenia, którą ręczne przepisywanie
     treści do wywołania MCP wprowadziło 2026-08-24 (`przeć imbir` zamiast całego
     zdania, w bloku, który sparsował się bez zarzutu). */
  if (items && items.has(id)) {
    const wCms = items.get(id).fieldData[POLE_JSONLD];
    if (wCms === undefined) {
      uwagi++;
      raport.push(`! ${zrodlo.meta.slug}: pole „${POLE_JSONLD}" nie istnieje w kolekcji`);
    } else if (!lustroCms) {
      uwagi++;
      raport.push(`! ${zrodlo.meta.slug}: brak pola-z-cms.json — nie mam z czego zregenerować bloku JSON-LD`);
    } else if (!lustroCms[id]) {
      uwagi++;
      raport.push(`! ${zrodlo.meta.slug}: itemu nie ma w pola-z-cms.json — odśwież lustro`);
    } else {
      const nasz = blok(dokument({ zrodlo, item: id }, lustroCms));
      if (nasz === wCms) zgodne++;
      else {
        rozjazdy++;
        raport.push(`✗ ${zrodlo.meta.slug} / ${POLE_JSONLD}: regeneracja ≠ CMS ` +
          `(${nasz.length} znaków vs ${String(wCms).length})`);
        if (pokaz) raport.push(pierwszaRoznica(wCms ?? '', nasz));
      }
    }
  }
