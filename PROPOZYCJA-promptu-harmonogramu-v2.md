# Propozycja: prompt zadania `tryb-gotowania-embed`, wersja 2

Powód: przebiegi 3 i 4 zrobiły po JEDNEJ jednostce i zakończyły się schludnie,
mając nietkniętą większość okna kontekstu. Przyczyna jest tekstowa i jest w dwóch
miejscach — w prompcie („Wykonaj «Następny krok» ze STAN.md", liczba pojedyncza)
i w skillu `ciaglosc-sesji` („5. Wykonuj «Następny krok»"). Skill poprawiony
2026-08-14 (nowy rozdział „Przebieg kończy się z POWODU, nie z uznania" + pętla
w szkielecie promptu). To jest druga kopia tej samej reguły, w prompcie.

**Uwaga o ryzyku.** `update_scheduled_task` z polem `prompt` zastępuje CAŁĄ treść.
Poniżej jest pełny tekst do wysłania, nie fragment — nie da się go zastosować
częściowo. Zmiany względem v1 są cztery i są zaznaczone niżej; reszta jest
przepisana znak w znak.

## Co się zmienia względem v1

1. **Nowy akapit „Pętla jednostek"** w nagłówku, zaraz pod akapitem o cykliczności.
2. **„## Co wykonać"** — pierwsze zdanie z „Wykonaj «Następny krok»" na pętlę
   + reguła planowania serii pomiarowej przed uzbrojeniem przeglądarki.
3. **„## Zakończenie"** — sprawdzian warunku wyjścia przed zamknięciem przebiegu.
4. **„Blokada Chrome → Wzięcie"** — odczyt i wzięcie jako dwa osobne wywołania
   (incydent z przebiegu 4: nadpisana cudza, dwuminutowa blokada, bo `cat`
   i przekierowanie stały w jednym poleceniu).

Nic poza tym nie jest ruszone: hierarchia prawdy, hashe, zakres pisania, zakazy
(staging, git, Figma), microcopy, konflikty otwarte — bez zmian.

---

## PEŁNA TREŚĆ DO WYSŁANIA
...
3. licznik przebiegów osiągnął 20;
...
2. **Warunki końca.** Przeczytaj STAN.md.
   ...
   - Licznik przebiegów osiągnął 20 przed zielenią → raport stanu MATRYCY, wyłącz zadanie, zakończ. Operator uzbroi kolejne ogniwo tym samym promptem ze świeżym licznikiem. To bezpiecznik ogniwa, nie granica zakresu.
...
Pętla jest W CAŁOŚCI LOKALNA. Powierzchnia pomiaru: `http://localhost:8123/harness/matrix.html` w Chrome operatora, przez narzędzia Claude-in-Chrome (serwer statyczny nad katalogiem łańcucha uruchamia operator; jeśli nie stoi — poproś i NIE próbuj `file://`, odrzucenie schematu jest udokumentowane w STAN.md). Jeśli narzędzia Chrome są odroczone, załaduj je JEDNYM wywołaniem ToolSearch:
...
HIERARCHIA PRAWDY: spec D1–D14 > WYMAGANIA/aneks > INTERAKCJE > surowa Figma. Zestaw Figmy jest prototypowo NIEOKABLOWANY — o zachowania pytaj INTERAKCJE, nigdy Figmy; Figma służy wyłącznie geometrii i weryfikacji wizualnej.
Luki buduj wg rekomendacji G1–G12 ze znacznikiem `// NIENARYSOWANE:` w kodzie. Konflikty rozstrzygnięte (C2, C5–C7, C9) wykonuj. Konflikty otwarte (C1 — pin BOTTOM, C8 — ramka `buttons`) zostaw operatorowi i NIE wprowadzaj do matrycy.
