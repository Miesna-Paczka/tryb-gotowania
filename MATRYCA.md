...
> ## ⛔ STATUSY PONIŻEJ SĄ NIEAKTUALNE OD 2026-08-15 (sesja interaktywna, po przeb. 38)
>
> **NIE CZYTAJ tej matrycy jako opisu stanu produktu, dopóki nie zostanie przebazowana.**
> Sześć poprawek zgłoszonych przez operatora zmieniło strukturę DOM na tyle, że
> **blok pomiarowy urywa się na 245 asercjach z 427** — pozostałe **182 nie zabierają
> głosu w ogóle**. Wiersz, który nie zabrał głosu, NIE JEST zielony; jest niemy.
> Zapisanie tego inaczej byłoby tą samą klasą fałszu co `80 = 0 + 80` z przeb. 26
> i `200/200` z przeb. 32.
>
> **Przyczyna jest znana i zamierzona, nie jest awarią:** ekran pełnej listy
> (`rysujListe()`) został USUNIĘTY decyzją operatora — rozwijanie „zobacz pozostałe"
> jest teraz akordeonem w miejscu. Sekcja `D` i wiersze `W59`–`W61` pytają o strukturę,
> która przestała istnieć.
>
> **Zmierzone jako padające przed urwaniem (10):** `B25`, `D5`, `D6` ×2, `D7`,
> `D11` ×3, `E6`, `W59`. Plus `I4`, które melduje wprost „blok przerwał się przed
> definicją sondy".
>
> **Do przebazowania:** 7 żywych selektorów `.mp-tryb__lista`, 22 asercje sekcji `D`,
> 3 wiersze `W59`–`W61`, 5 użyć `przelaczListe`/`listaOtwarta`. Co najmniej jeden
> punkt urwania poza tym, który zabezpieczono `wezelLub`/`elLubZ` w linii ~2446.
>
> **Sekcja `S` (staging) też jest nieaktualna** w drugą stronę: mierzona była na
> `@v1.0.0-rc.1`, a produkt lokalny jest od niej o sześć poprawek do przodu.
> `S4` na pewno się ruszyło — operator uzupełnił pola CMS (przepis ma 9 kroków
> z treścią, zmierzone).
...
| F2b | **`×` jest OSIĄGALNY palcem**: `document.elementFromPoint` w geometrycznym środku `.mp-tryb__zamknij` zwraca ten przycisk (albo jego potomka), a kliknięcie w element ZWRÓCONY przez trafienie otwiera `S2` | I-07 · zgłoszenie operatora nr 1 (hand-off §5) | 1× | DOM | 🟢 **ZAMKNIĘTY NA ZIELONO NA STAGINGU 2026-08-16, build `4c157ff` przez GitHub Pages: `belka z-index: 2`, `elementFromPoint` w środku `×` → `TRAFIA`; `wstecz` i `dalej` też `TRAFIA`** [V]. Diagnoza i naprawa powstały wcześniej tego dnia na `@5be768d`** [V]. **Powód założenia: `F2` był zielony przez trzydzieści przebiegów, a przycisk był NIEKLIKALNY.** Zmierzone `document.elementsFromPoint` w środku `×`: stos od wierzchu `DIV.mp-tryb__top` (658×668) → `BUTTON.mp-tryb__zamknij` → `DIV.mp-tryb__belka`; `trafia: false`. Kolejność dzieci korzenia zmierzona: `belka`(0) · `top`(1) · `bottom`(2) · `scrim`(3, `z-index:4`) — wszystkie poza scrimem z `z-index:auto`, więc o trafieniu decyduje kolejność w drzewie, a `.mp-tryb__top` ma `position:absolute;inset:0` i przykrywa CAŁY overlay, w tym belkę. **BOTTOM stoi w drzewie PO TOP-ie, dlatego jego przyciski działały** (`akcjaPrimary` i `akcjaGhost` zmierzone `trafia: true` w tym samym pomiarze) — i to jest cała przyczyna, dla której objaw wyglądał na „tylko iks nie działa”. **Dlaczego `F2` tego nie widział: metodą było `oko`, a pomiar programowy wołał `element.click()`, który OMIJA trafianie w punkt** — mechanizm meldował `dialog: true` i wyglądał na sprawny. To ta sama klasa fałszywej zieleni co `80 = 0 + 80`: asercja pytała „czy handler jest podpięty”, a operator pytał „czy da się w to trafić”. **Naprawa: `z-index:2` na `.mp-tryb__belka`** (nie `1` — `.mp-tryb__ptaszek` ma `z-index:1`, a `.mp-tryb__top` nie tworzy kontekstu układania, więc ptaszki uczestniczą w kontekście korzenia i przy `1` wygrałyby remis kolejnością w drzewie, wchodząc NA belkę zamiast pod jej rozmycie). Powyżej zostają tooltip (3) i scrim dialogów (4) — oba mają być nad belką. **Przemiar po wstrzyknięciu reguły na żywo, ta sama sesja, ta sama ramka: `trafia: true`, wierzch `BUTTON.mp-tryb__zamknij`, kliknięcie w element zwrócony przez `elementFromPoint` → `dialog.rodzaj() === 'S2'`** [V]. **Wiersz mierzy trafienie, nie podpięcie — asercja wołająca `.click()` na referencji NIE zamyka tego wiersza i nie wolno jej tak napisać.** Do domiaru w harnessie: ten sam predykat dla wszystkich sterowników na 7 ramkach (obecny pomiar jest 1× i pochodzi ze stagingu, nie z harnessu, bo serwer na 8123 nie odpowiadał) | **39** |
...
| W42b | wiersz `stan=zużyty` niesie **WYPEŁNIONE pudełko checkboxa** (tło `primary-text` #3E2B22, znak `✓` bielą pełną #FFFFFF, DM Sans SemiBold 600, 10 px/1,5), a nie samo przekreślenie nazwy | `7273:10878` → `I7273:10878;7224:10918` (odczyt `get_design_context` 2026-08-16) · render `7196:10993` | 1× | DOM | 🟢 **ZMIERZONY NA STAGINGU 2026-08-16 po publikacji, na krokach 3 · 5 · 7 · 9, wynik identyczny na każdym** [V]: wiersz `zużyty` ma pudełko `rgb(62,43,34)` = #3E2B22, znak `rgb(255,255,255)` = biel pełna, obrys atramentowy, a nazwa `text-decoration-line: line-through` — **obie delty naraz, zgodnie z `7273:10878`**. Kontrola przeciwna na tej samej ramce: wiersz `teraz` ma tło `rgba(0,0,0,0)` i znak `rgba(0,0,0,0)`, czyli pudełko puste — gdyby wypełnienie było bezwarunkowe, ta kontrola by padła. Rozstrzygnięcie operatora D-39.4 („zużyte wymagają zarówno odhaczenia checkboxa, jak i przekreślenia tekstu") Zmierzone w runtimie: wypełnienie pudełka daje wyłącznie reguła `.mp-tryb__wiersz[data-odhaczony] .mp-tryb__ptaszek`; przy `[data-stan="zuzyty"]` pudełko zostaje PUSTE (`background:transparent`, `color:transparent`), a delta to `text-decoration:line-through` na nazwie. Figma na TYM SAMYM węźle, który `W42` podaje jako źródło, rysuje pudełko wypełnione **i** nazwę przekreśloną — obie rzeczy naraz; render klatki `7196:10993` potwierdza to na pięciu wierszach sekcji „zużyte". **`W42` jest zielony i prawdziwy — pyta o KOLOR NAZWY i tylko o niego.** Ten wiersz jest tym, o co `W42` nie zapytał, i to jest wzorzec, nie wypadek: wiersz zbudowany wokół jednej własności przechodzi także wtedy, gdy sąsiednia własność tego samego węzła jest zepsuta. **Dlaczego nie zamykam tego sam, mimo reguły „wygląd rozstrzyga Figma": `W42` i notatka `G2` przy regule CSS to zapisane rozstrzygnięcie STANU, nie odczyt** — `G2` mówi wprost, że zlanie „odhaczony teraz" z „zużyty wcześniej" odbiera ekranowi `S1` czytelność, a tego argumentu Figma nie unieważnia, bo klatka `S1` pokazuje inny moment. **Do rozstrzygnięcia przez operatora:** czy zużyty ma mieć wypełnione pudełko (zgodnie z klatką), czy `G2` ma pierwszeństwo i klatka jest uproszczeniem. Do tego czasu wiersz zostaje CZERWONY — zieleń bez rozstrzygnięcia byłaby zieleń z uznania | **39** |
| W25b | kreska wywoływacza „zobacz pozostałe" ma **jeden** kolor niezależnie od kontekstu: `primary-text` #3E2B22 zarówno wewnątrz `.mp-tryb__ramka-skladnikow`, jak i poza nią | `7196:10997` · `7196:11013` (odczyt 2026-08-16) · **D-39.5 (operator)** | 5× | DOM | 🟢 **ZMIERZONY NA STAGINGU 2026-08-16 po publikacji: `border-top-color` wywoływacza = `rgb(62,43,34)` = #3E2B22 na krokach 3 · 5 · 7 · 9** [V]. **Zastrzeżenie do wiersza, nie do produktu:** w obecnym układzie `.mp-tryb__wiecej` renderuje się WYŁĄCZNIE wewnątrz `.mp-tryb__ramka-skladnikow` (`wWRamce: true` na każdej zmierzonej ramce), więc drugi kontekst nie istnieje i pytanie o RÓWNOŚĆ obu kontekstów jest dziś niesprawdzalne. Zmierzona jest wartość, nie równość. Zmiana usuwa rozjazd LATENTNY — taki, który wyszedłby przy pierwszym wywoływaczu postawionym poza ramką. Pierwotna czerwień Zmierzony defekt: reguła bazowa `.mp-tryb__wiecej` dawała `border-top` `beige-2` #C5B18A, a zawężenie `W25` nadpisywało go atramentem WYŁĄCZNIE wewnątrz ramki składników — ta sama kreska miała więc dwa kolory zależnie od rodzica i to jest zgłoszenie operatora nr 3 w całości. Atrament stoi teraz w regule BAZOWEJ, zawężenie zostaje przy samym rytmie (`margin-top:0`). `.mp-tryb__linia` (kreska listy pełnej) miała atrament już wcześniej — po zmianie wszystkie trzy kreski są zgodne. **Wiersz mierzy RÓWNOŚĆ obu kontekstów, nie samą wartość** — inaczej przeszedłby także wtedy, gdyby oba były beżowe | **39** |
...
|---|---|---|---|
| S1 | pływające CTA jest na stronie przepisu i **kliknięcie OTWIERA overlay** | 🔴 **ROZSZCZEPIONE NA DWA FAKTY, przeb. 38 — i to jest inne ustalenie niż w przeb. 35.** (a) **OKABLOWANIE DZIAŁA** [V]: `[data-mp-gotowanie-start].click()` w ramce 360 px otwiera overlay — `#mp-tryb` pojawia się w drzewie z `data-otwarty`, dzieci `body` 39 → 41, `MP.tryb.ostrzezenia()` puste. Diagnoza z przeb. 35 („CTA to cichy no-op") jest **nieaktualna**: element jest dziś `<div data-mp-gotowanie-cta>` z `<a data-mp-gotowanie-start href="#tryb-gotowania">` w środku i nasłuch odpowiada. (b) **ELEMENT JEST NIEOSIĄGALNY DLA UŻYTKOWNIKA** [V]: `.recipe-floating-cta` ma `display:none` w regule BAZOWEJ arkusza `-ea5c01.webflow.shared.*.min.css`, powtórzone w `@media (max-width:991px)` i `@media (max-width:767px)`; `@media (max-width:479px)` nie ustawia `display` wcale. **Zmierzone na dziewiętnastu szerokościach 320–1024: `display:none` na KAŻDEJ** — nie ma progu, jest stała. Żadne przewijanie tego nie zmienia (sprawdzone na siedmiu pozycjach 0–4000 px). Wiersz zostaje czerwony, bo pyta o ścieżkę użytkownika, a nie o nasłuch. **Właściciel: sesja równoległa / operator w Designerze — łańcuchowi zapis w Webflow jest zabroniony** | 35 · **38** |
| S2 | runtime i parser wczytują się bez błędu, **parser przed runtime'em** | 🟢 **ZIELONE, przeb. 38** [V]. Oba skrypty są na stronie, z jsDelivr, **w wiążącej kolejności**: `…/tryb-gotowania@v1.0.0-rc.1/przepis-parser.min.js` PRZED `…@v1.0.0-rc.1/tryb-gotowania.min.js`. `window.MP`, `MP.przepis`, `MP.tryb` to obiekty; `MP.tryb` wystawia komplet API (25+ pozycji), `MP.przepis` 14. **SHA pomiaru: tag `v1.0.0-rc.1` = commit `37cc7b8`** — adres w `src` niesie tag, więc wynik jest przypisany do wersji, a nie do ruchomego `main`. Zapis z przeb. 35 („żadnego z nich na stronie nie ma") dotyczył stanu sprzed wklejenia embedów przez operatora | **38** |
| S3 | konsola gospodarza: **zero błędów i ostrzeżeń POCHODZĄCYCH Z EMBEDU** | 🟢 **ZIELONE NIETRYWIALNIE, przeb. 38** [V] — i to jest różnica wobec przeb. 35, gdzie ten sam wynik był prawdziwy tylko dlatego, że embedu na stronie nie było. Dziś runtime i parser DZIAŁAJĄ, overlay był otwierany i zamykany, a konsola ma **zero błędów i zero ostrzeżeń**. Dwa wpisy `log`, oba CUDZE i notowane osobno: `GTM user_type = guest` oraz `[mp-mnav] Storefront routing: staging fallback…` | 35 · **38** |
| S4 | kontrakt DOM z §5 pakietu **w komplecie**, brak opisany NAZWĄ pola | 🔴 **BEZ ZMIAN wobec przeb. 35, przemierzone w przeb. 38** [V]. Obecne, ale PUSTE (0 znaków, klasa `w-dyn-bind-empty`): `data-mp-skladniki` jako `div.recipe-ing__source`, `data-mp-kroki` jako `div.recipe-steps__source`. **Brak w ogóle, wymienione nazwami:** `data-mp-pole`, `data-mp-surowe`, `data-mp-przepis`, `data-mp-id`, `data-mp-porcje`. `w-dyn-bind-empty` to sygnatura Webflow — wiązanie CMS w szablonie JEST, pole w rekordzie PUSTE, czyli brak TREŚCI, nie brak szablonu. **Robota pipeline'u treści i operatora, nie kodu** | 35 · **38** |
| S5 | font ikon z CDN Webflow **renderuje ligatury** (sonda `I4` z kontrolą ujemną) | 🟢 **POTWIERDZONE PONOWNIE w przeb. 38, z kontrolą ujemną** [V]. Trzy `@font-face` Material Symbols Outlined (300/400/500) z `cdn.prod.website-files.com/6983617613052dc9fe624303/…`, wszystkie `status: loaded`. **Ligatury renderują się:** `soup_kitchen` i `hourglass` przy 20 px mają **dokładnie 20,0 px szerokości** (jeden glif), a **kontrola ujemna** — napis `nieistniejacy_glif_xyz` — **425,6 px**, czyli litery. Bez kontroli ujemnej wiersz przechodziłby na stronie, na której ligatury nie działają wcale | 35 · **38** |
| S6 | próg 500 px zachowuje się **tak samo jak lokalnie** | 🔴 **NIEMIERZALNY, i to jest OSTRZEJSZE ustalenie niż rozjazd 1 px z przeb. 35** [V]. Progu nie ma czego mierzyć: `.recipe-floating-cta` jest `display:none` na WSZYSTKICH dziewiętnastu zmierzonych szerokościach (320 · 360 · 390 · 440 · 480 · 498 · 499 · 500 · 501 · 520 · 600 · 700 · 767 · 768 · 800 · 900 · 991 · 992 · 1024), więc funkcja widoczności jest stałą, nie schodkiem. Pomiar z przeb. 35 (498/499/500 `flex`, od 501 `none`) **przestał być odtwarzalny** — element zmieniono w sesji równoległej między przeb. 35 a 38. Harness `prog.html` w tym samym przebiegu: 499 widoczny / 500 ukryty, `zgodne: true` [V]. **Pozycja decyzyjna D-35.1 (500 czy 501) jest przez to BEZPRZEDMIOTOWA do czasu przywrócenia widoczności CTA** | 35 · **38** |
| S7 | rozjazd wobec pomiaru lokalnego — **lista sprawdzonych wielkości**, nie zdanie „bez rozjazdów" | 🔴 **PIERWSZE PORÓWNANIE NA ŻYWYM RUNTIMIE, przeb. 38** — poprzednio overlay był NIEPORÓWNYWALNY, bo go na stronie nie było. Ramka 360×780, trik same-origin. **ZGODNE co do wartości (11):** `position` overlaya `fixed` · `z-index` **2147483000** · tło overlaya `rgb(255,253,251)` · tło belki `color(srgb 1 0.992157 0.984314 / 0.8)` · `position` BOTTOM-u `absolute` · tło BOTTOM-u `rgb(255,255,255)` · dopełnienie TOP-u **88/80 px** · `--mp-bottom-h` = **80px** publikowane na korzeniu overlaya (B26) · `--mp-beige-1` **#F1ECDF** · `--mp-beige-2` **#C5B18A** · rodzina kroju etykiety `"DM Sans", system-ui, sans-serif`. **Znak marki przeżył drogę repo → jsDelivr → Webflow: slot 51×40, `svg` 1, `img` 0 — identycznie jak `B24` lokalnie** [V]. **ROZJAZD 1 — `MP.tryb.czesci()` oddaje 16 kluczy zamiast 23.** Brakuje: `porcjeIle`, `porcjeMinus`, `porcjePlus`, `kartaS1Tor`, `kartaS1Wyp`, `kartaS1`, `kartaKoniec`. Hipoteza [I]: to skutek PUSTEGO rekordu CMS (S4) — węzły ekranu startowego i selektora porcji nigdy nie powstały. **Nie do rozstrzygnięcia przed uzupełnieniem pól**, więc nie wolno tego zapisać ani jako zgodność, ani jako defekt runtime'u. **ROZJAZD 2 — strona gospodarza definiuje własną zmienną `--mp-belka-h: 64px` na `documentElement`; harness nie ma jej wcale, a belka mierzy 72 px na obu.** Kolizja PRZESTRZENI NAZW `--mp-*` między embedem a serwisem: dziś nieszkodliwa (runtime tej zmiennej nie konsumuje), ale `I7` pilnuje czystości `--mp-*` wyłącznie WEWNĄTRZ artefaktu i takiej kolizji z definicji nie zobaczy. **Pozycja decyzyjna dla operatora** | 35 · **38** |
...
| sekcja | wierszy | 🟢 | 🔴 | ⏸ |
|---|---|---|---|---|
| A · parser i model | 16 | **16** | 0 | 0 |
| B · układ i geometria | 26 | **26** | 0 | 0 |
| C · minutniki | 17 | **17** | 0 | 0 |
| D · lista składników | 12 | **12** | 0 | 0 |
| E · zamienniki i tooltip | 14 | **14** | 0 | 0 |
| F · nawigacja i stany | 15 | **15** | 0 | 0 |
| G · porcje, wejścia, progi | 11 | **11** | 0 | 0 |
| H · testy negatywne | 12 | **12** | 0 | 0 |
| I · higiena | **13** | **13** | 0 | 0 |
| W · wykończenie powierzchni | 78 | **73** | 0 | **5** |
| **RAZEM (pętla lokalna)** | **214** | **209** | **0** | **5** |
| **S · bramka stagingowa** (poza zielenią lokalną, W ŚRODKU warunku 2) | **7** | **3** | **4** | 0 |
| Z · poza pętlą | 9 | — | — | — |

**Przeliczone maszynowo w przeb. 38** przez zliczenie PIERWSZEGO znacznika statusu
w każdym wierszu o identyfikatorze — nie utrzymywane ręcznie. Poprzednia wersja tej
tabeli stała na `204 / 203 / 1` i była nieaktualna od przeb. 30 (patrz akapit niżej);
liczba `8` w wierszu `I` pochodziła sprzed założenia `I9`–`I13`.

**Pętla lokalna nie ma ani jednej czerwieni.** Pięć `⏸` w sekcji `W` to pozycje
wstrzymane decyzją operatora (`W18`, `W46`, `W47`, `W77`, `W79`), nie niezmierzone.
**Wszystkie cztery czerwienie sekcji `S` czekają na czynność operatora albo sesji
równoległej** — `S1` i `S6` na przywrócenie widoczności `.recipe-floating-cta`
w Designerze, `S4` na uzupełnienie pól CMS, `S7` na jedno i drugie. Łańcuchowi zapis
w Webflow jest zabroniony, więc żaden kolejny przebieg tych wierszy nie ruszy.
