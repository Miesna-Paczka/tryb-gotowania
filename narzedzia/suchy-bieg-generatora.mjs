...
import { kontrolujWynik, kontrolujZrodlo } from '../lancuch-html/kontrole.mjs';
...
/* --- 7c: mikroskładnia odżywcza, która daje PUSTĄ tabelę ------------------
   ZASTĄPIŁO fiksturę z tabelą w `wartosci-100-html`. Tamta podstawiała pod
   kontrolę HTML, który generator produkował do 2026-08-19 — a pole
   `wartosci-100-html` przestało istnieć 2026-08-24 razem z całą tą drogą
   renderu. Fikstura pilnowałaby wtedy ścieżki, której nie ma: przechodziłaby
   na zielono niezależnie od stanu produktu.

   Nowa droga jest atrybutowa (`data-mp-odz-zrodlo-*` → `mpOdzywczeEmbed 2.1.0`),
   więc zdejmowania znaczników już nie ma. Został jeden tryb awarii i jest cichy:
   klucze z PUSTYMI wartościami. Kontrole źródła go nie widzą — dwukropki są,
   pole nie jest puste — a embed renderuje tabelę bez ani jednego wiersza,
   czyli blok o poprawnej geometrii i zerowej treści.

   Zmierzone 2026-08-24: to JEDYNY przypadek, w którym ta kontrola odpala sama.
   Przy tabeli, pustym polu i samych średnikach ubiega ją `kontrolujZrodlo`
   („człon bez dwukropka", „puste") — dlatego fikstura celuje właśnie tutaj,
   a nie w przypadek, który i tak złapałby ktoś inny. */
{
  const puste = { ...CALY, pola: { ...CALY.pola, 'wartosci-porcja': 'energia: ; tłuszcz: ; białko: ' } };
  const r = kontrolujWynik(puste, zbuduj(WZORZEC, puste));
  const zrodlowe = kontrolujZrodlo(puste);
  if (!r.bledy.some((b) => b.includes('zero członów'))) {
    oblane++;
    console.log('✗ klucze z pustymi wartościami PRZESZŁY kontrolę');
    console.log(r.bledy.length ? r.bledy.map((b) => `    dostałem: ${b}`).join('\n') : '    dostałem: ZERO BŁĘDÓW');
  } else if (zrodlowe.bledy.length) {
    /* Gdyby kontrola źródła zaczęła to łapać, powyższa asercja przechodziłaby
       nadal — i nie mierzyłaby już nic. Wtedy tę kontrolę trzeba usunąć albo
       przecelować, a nie zostawiać jako ozdobę. */
    oblane++;
    console.log('✗ przypadek przestał być wyłączny — łapie go już kontrola źródła: ' + zrodlowe.bledy[0]);
  } else {
    zdane++; console.log('✓ klucze z pustymi wartościami — kontrola widzi pustą tabelę odżywczą');
  }
}
