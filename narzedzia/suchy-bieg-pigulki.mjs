...
/* Import tolerancyjny: na maszynie lokalnej playwright stoi pod ścieżką bezwzględną
...
/* D-47.1 — MINUTNIK STARTUJE ZWINIĘTY, więc przyciski kafla są schowane pod
   `overflow:hidden` i `height:0`. Do 2026-08-23 ta próba klikała w nie mimo to:
   `display` zwiniętego wiersza NIE jest `none`, więc filtr widoczności je przepuszczał,
   a `primary` — jedyny z `display:none` w tej formie — dawał „brak przycisku".
   Stąd jedna czerwień prawdziwa (primary) i dwie zielenie FAŁSZYWE (ghosty klikane
   w stanie, w którym użytkownik ich nie widzi). Rozwijamy jawnie przed każdym tapem
   w przycisk kafla — dokładnie tak, jak zrobiłby to palec. */
const rozwin = (k) => k.evaluate(() => {
  const p = [].slice.call(document.querySelectorAll('.mp-tryb__pigulka'))
    .find((x) => x.getAttribute('data-forma') !== 'start');
  if (!p) return 'brak kafla';
  if (p.getAttribute('data-forma') === 'zwinieta') p.querySelector('.mp-tryb__wiersz-min').click();
  return p.getAttribute('data-forma');
}).then((r) => k.waitForTimeout(320).then(() => r));
...
// ── 1. BIEGNIE: skład i brzmienia ───────────────────────────────────────────
{
  ...
  /* D-46.2 — brzmienie bez rzeczownika. Do 2026-08-20 primary składał się z nazwy
     minutnika („✓ kurczak gotowy"), co przy „brokuły" dawało „brokuły gotowy".
     Ustalenie: „na wypełnionym CTA ma być tylko tekst »gotowe«". Próba stała czerwona
     od tamtej zmiany, bo trzymała stare brzmienie — i to jej wzorzec `tapnij`
     przestał trafiać w przycisk, stąd dodatkowo dwa „pigułka zniknęła". */
  spr(s.primary.txt === '✓ gotowe', 'primary = „✓ gotowe"', `jest: ${JSON.stringify(s.primary.txt)}`);
...
// ── 2. BIEGNIE: primary zamyka ──────────────────────────────────────────────
{
  const k = await karta();
  await rozwin(k);
  await tapnij(k, 'gotowe'); const s = await czytaj(k);
  console.log('\n[2] biegnący — primary „✓ gotowe" zamyka minutnik (I-33)');
...
// ── 4. ZERO: skład i brzmienia ──────────────────────────────────────────────
{
  ...
  spr(s.primary.txt === '✓ gotowe', 'primary bez zmian: „✓ gotowe"', `jest: ${JSON.stringify(s.primary.txt)}`);
