...
| I-15 | tap pigułki — rozwinięcie | `7195:11088` (pigułka 328×40) → `7211:10893` (blok 328×236) | — | [V] geometria obu · [I] wyzwalacz; glif `keyboard_arrow_down` (`7211:10928`) — **od 2026-08-20 to glif BAZOWY, stały w obu stanach**, kierunek robi obrót (I-36) | Aneks 4 |
| I-16 | tap nagłówka — zwinięcie | rozwinięty → pigułka | — | [V] glif `keyboard_arrow_up` (`7240:10921`) w S5 · [I] kierunek. **SKORYGOWANE 2026-08-20:** `up` nie jest osobnym glifem, tylko `keyboard_arrow_down` obróconym o −180° (I-36). Adnotacja o tym, że klatki z `down` to „dryf Figmy”, była nadinterpretacją: `7211:10928` (`down`) i `7240:10921` (`up`) to ten sam element w dwóch końcach przejścia | Aneks 4 |
...
| I-36 | szewron pigułki — **obrót, nie podmiana glifu** | zwinięta `rotate(0deg)` → strzałka w DÓŁ · rozwinięta (`krotka` i `pelna`) `rotate(-180deg)` → strzałka w GÓRĘ; przejście `transform 280ms`, przy `prefers-reduced-motion: reduce` bez animacji, obrót zostaje | — | [U] decyzja operatora 2026-08-20: powtórzyć schemat akordeonów ze stron produktowych. Wzorzec **zmierzony na żywej produktówce**, nie z pamięci: `.mp-faq-item__heading .icon-text.is-faq` — `transition:transform 280ms`, `[open] → rotate(-180deg)`, origin w środku pudełka. Znak `-180`, nie `+180`: `mp-mnav__group-chevron` obraca `+180deg` przez 240 ms i jest w witrynie odosobniony. **Pociąga zdjęcie R10** — szewron musi być widoczny także przy zwiniętej, inaczej pozycji „w dół” nikt nie zobaczy | Produktówka · D-40.1 |
| I-37 | wywoływacz listy składników — **ten sam mechanizm co pigułka** | „zobacz pozostałe" `rotate(0deg)` → strzałka w DÓŁ · „zwiń" `rotate(-180deg)` → strzałka w GÓRĘ; glif stały `keyboard_arrow_down`, przejście `transform 280ms` | — | [U] decyzja operatora 2026-08-20, ujednolicenie po I-36. Do tego dnia było to **jedyne miejsce w nakładce podmieniające dwa glify** (`up`/`down`), z adnotacją D-39.32 „dwa glify, nie obrót". Stanem steruje `aria-expanded`, które przycisk już nosił — nie powstaje drugi kanał tej samej prawdy. **Skutek dla zbioru ligatur:** `keyboard_arrow_up` nie jest już rysowany nigdzie i wypada z `LIGATURY` (13 → 12), bo zbiór opisuje UŻYCIE, nie katalog subsetu | D-40.3 |
| I-38 | odmowa trzeciego minutnika NIESIE ŻĄDANIE | tap „zakończ i włącz «‹nazwa›»" w S4 kończy wskazany minutnik **i uruchamia ten, o który proszono**; każde inne wyjście z okna (CTA „wróć do gotowania", zamknięcie dialogu) gasi żądanie | — | [U] decyzja operatora 2026-08-20. Do tego dnia odmowa zabierała żądanie ze sobą, a treść okna obiecywała wprost „żeby zrobić miejsce na kolejny" — użytkownik musiał poprosić drugi raz, bez żadnej wskazówki na ekranie. Żądanie żyje **w obiekcie dialogu**, więc `zamknijDialog()` je niszczy i nie ma stanu, który mógłby zwietrzeć. Etykieta niesie obie połowy skutku, bo łączenie ich pod gołym „zakończ" zamieniałoby przypadkowe tapnięcie w podwójną stratę. Limit dwóch (I-18/D11) bez zmian | D-40.5 |
